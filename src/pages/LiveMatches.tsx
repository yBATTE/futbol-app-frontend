...
const GoalsList: React.FC<{ goals: GoalType[]; teamId: string; teamName: string }> = ({ goals, teamId, teamName }) => {
  ...
  return (
    <div className="space-y-2">
      <h4 className="font-semibold text-gray-800 flex items-center space-x-2">
        ...
        <span>Goles de {teamName}</span>
      </h4>
      {teamGoals.map((goal) => (
        <div key={goal._id} className="bg-green-50 border border-green-200 rounded-lg p-3">
          <div className="flex items-center justify-between">
            <div>
              <div className="font-medium text-green-800">
                #{goal.player.number} {goal.player.firstName} {goal.player.lastName}
              </div>
              {goal.assist && (
                <div className="text-sm text-green-600">
                  Asistencia: #{goal.assist.number} {goal.assist.firstName} {goal.assist.lastName}
                </div>
              )}
            </div>
            <div className="text-right">
              <div className="font-bold text-green-700">{goal.minute}'</div>
              <div className="text-xs text-green-600">
                {new Date(goal.createdAt).toLocaleTimeString("es-ES", {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </div>
